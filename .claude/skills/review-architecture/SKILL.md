...
description: Review codebase for layer violations, import direction, naming conventions, and file placement
context: fork
agent: Explore
allowed-tools:
  - Read
...
1. `$SKILL_DIR/ref/layer-rules.md` — Layer diagram and import matrix
2. `$SKILL_DIR/ref/import-rules.md` — Grep patterns for detecting violations
...
- [ ] Check for cross-domain imports within the same layer (e.g., portfolio_repo importing from sprint_repo)

### 3. SQL Placement
- [ ] No SQL strings in `pages/` files
- [ ] No SQL strings in `services/` files
- [ ] All SQL lives in `utils/data_access.py` or `repositories/` files
- [ ] No raw SQL in `callbacks/` files
...
### 5. File Placement
- [ ] Chart builders are in `utils/charts.py` or `charts/` directory, not in pages
- [ ] Data access functions are in `utils/data_access.py` or `repositories/`, not in pages
...
- [ ] Every table in `schema_ddl.sql` has `created_at TIMESTAMP NOT NULL DEFAULT current_timestamp()`
...
### 7. Soft Deletes & Audit
- [ ] Tables that support deletion use `is_active` or `deleted_at` (not hard deletes)
- [ ] Audit-critical tables have `'delta.enableChangeDataFeed' = 'true'`
...
| ID naming convention | PASS/FAIL | ... |
| ... | ... | ... |
```

Severity classification:
- **CRITICAL**: Layer violations that create circular dependencies or tight coupling
